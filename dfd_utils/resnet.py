...
class StemCIFAR(nn.Sequential):
    # Inspired by: https://github.com/ae-foster/pytorch-simclr/blob/simclr-master/models/resnet.py
    # and our resnet. Basically, kernel size is smaller as well as stride and padding. Not sure about the sk>0
    # also removed the maxpooling
    def __init__(self, sk_ratio, width_multiplier):
        ops = []
        channels = 64 * width_multiplier // 2
        if sk_ratio > 0:
            # not sure about this, maybe remove?
            ops.append(conv(3, channels, kernel=3,stride=1))
            ops.append(BatchNormRelu(channels))
            ops.append(conv(channels, channels))
            ops.append(BatchNormRelu(channels))
            ops.append(conv(channels, channels * 2))
        else:
            ops.append(conv(3, channels * 2, kernel_size=3, stride=2,bias=False))
        ops.append(BatchNormRelu(channels * 2))
        ops.append(nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
        super().__init__(*ops)

class ResNet(nn.Module):
    def __init__(self, layers, width_multiplier, sk_ratio, cifar):
        super().__init__()
        if cifar:
            stem_func = StemCIFAR
        else:
            stem_func = Stem
        ops = [stem_func(sk_ratio, width_multiplier)]
...
def get_resnet(depth=50, width_multiplier=1, sk_ratio=0, cifar=False, head_layers=3):  # sk_ratio=0.0625 is recommended
    layers = {18 : [1, 1, 1, 1], 50: [3, 4, 6, 3], 101: [3, 4, 23, 3], 152: [3, 8, 36, 3], 200: [3, 24, 36, 3]}[depth]
    resnet = ResNet(layers, width_multiplier, sk_ratio, cifar)
    return resnet, ContrastiveHead(resnet.channels_out, head_layers)
