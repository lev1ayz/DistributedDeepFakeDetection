...
class BaseSSL(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        if not 'faceforensics_path' in vars(hparams):
            ...
            print(f'loading {hparams.faceforensics_path}')
    ...
    def prepare_data(self):
        ...
        # TODO add FaceForensics
            ...
            test_dataset, _ = torch.utils.data.random_split(ff_ds_test, [test_size, ts_size],
                                                             generator=torch.Generator().manual_seed(self.hparams.seed))
            train_subset = Subset(train_dataset, list(range(0, min(28000,len(train_dataset)))))
            test_subset = Subset(test_dataset, list(range(0, min(4000, len(test_dataset)))))
